...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states, tgt_pos=None, tmp_score=None, imp_pos=None, imp_op=None,dense_layer_output=False):
        dense_hidden_states = self.dense(hidden_states)
        hidden_states = self.intermediate_act_fn(dense_hidden_states)  # [batch, max_len, nslot]
        ...
        if imp_op == 'return':
            if dense_layer_output :
                return hidden_states, imp_weights, dense_hidden_states
            return hidden_states, imp_weights
        else:
            if dense_layer_output :
                return hidden_states, dense_hidden_states
            return hidden_states
...
class BertForMaskedLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, tgt_pos=None, tgt_layer=None, tmp_score=None, tgt_label=None, imp_pos=None, imp_op=None, return_all_logits=False):
        if tmp_score is not None:
